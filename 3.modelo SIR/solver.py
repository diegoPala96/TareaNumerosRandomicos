...
class Learner(object):
    ...
    def train(self):
        recovered = self.load_recovered(self.country)# recuperados
        death = self.load_dead(self.country)# muertos
        data = (self.load_confirmed(self.country) - recovered - death)#confirmados
        #print('data',data)
        #print('recuperados', recovered)
        #recovered=np.array([0])
        print('loss',self.s_0,self.i_0,)
        optimal = minimize(loss, [0.001, 0.001], args=(data, recovered, self.s_0, self.i_0, 500), method='L-BFGS-B', bounds=[(0.00000001, 0.4), (0.00000001, 0.4)])
        print(optimal)
        ...
        fig.savefig(f"{self.country}CONRECUPERADOS.png")
